...
var numCrawler = flag.Int("n", 8, "set the maximum number of crawlers here!")
...
func main() {
	...
	if len(flag.Args()) > 0 {
		for _, job := range flag.Args() {
			...
			r := filesize.Request{root, job, make(chan struct{}, *numCrawler)}
		...
		r := filesize.Request{root, ".", make(chan struct{}, *numCrawler)}
		forest = append(forest, r.Root)
		wg.Add(1)
		go filesize.DirCrawl(r, *force, &wg)
	}
